...
using UnityEngine;
using UnityEngine.UI;  // For UI elements (radial cooldown image)
...
public class HardBullet : MonoBehaviour
{
    ...
    // UI Cooldown Elements
    public Image cooldownImage; // Radial Image to display cooldown (assigned in Inspector)
    ...
    private void Start()
    {
        ...
        {
            Debug.LogError("Weapon: No cooldownImage assigned in Inspector!");
        }
    }
    ...
    private void Update()
    {
        ...
        else
        {
            ...
            // Update the cooldown UI (radial image) to show cooldown progress
            cooldownImage.fillAmount = 1 - (timeBtwShots / startTimeBtwShots);
        }
    }
